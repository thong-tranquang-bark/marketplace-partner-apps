...
import { COMMON_NAME_FIELDS, MAX_LEVEL, MORE_PLACEHOLDER, ASSET_CONTENT_TYPES, DEFAULT_LOCALE } from './constant';
 ...
 */
export function buildTreeStructure(referencesTree: Record<string, EntryProps>, parentEntryId: string, path: string[] = [], level: number = 0): TreeNode | null {
 ...
 */
export function findParentIds(tree: TreeNode, targetEntryId: string, currentPath: string[] = []): string[] {
 ...
 */
export function collectDisabledPaths(node: TreeNode, listBlockContentIds: string[], path: string = '', hasBlockedAncestor: boolean = false): Set<string> {
  ...
  node.children.forEach((child) => {
    const childDisabledPaths = collectDisabledPaths(child, listBlockContentIds, nodePath, isDisabled);
